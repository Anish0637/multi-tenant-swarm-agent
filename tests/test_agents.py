...
from agents import (
    HRAgent, FinanceAgent, MedicalAgent, SupervisorAgent,
...
def _make_task(task_type: str, payload: dict = None,
               tenant_id: str = "test", context: dict = None) -> TaskRequest:
    return TaskRequest(
        tenant_id=tenant_id,
        task_type=task_type,
        payload=payload or {},
...
def test_result_from_state_happy():
    state: AgentGraphState = {
        "task_id": "abc", "tenant_id": "t", "task_type": "x",
        "payload": {}, "priority": 5, "user_id": "u",
        "status": "success", "result": {"foo": "bar"}, "error": None,
...
def test_result_from_state_with_error():
    state: AgentGraphState = {
        "task_id": "xyz", "tenant_id": "t", "task_type": "y",
        "payload": {}, "priority": 5, "user_id": "u",
        "status": "failed", "result": {}, "error": "boom",
        "created_at": "now", "updated_at": "now",
        "retry_count": 1, "metadata": {},
    }
    r = result_from_state(state)
...
async def test_hr_process_leave():
    ...
    result = await agent.handle_task(
        _make_task("process_leave",
                   {"employee_id": "E01", "leave_type": "annual", "days": 5})
    )
...
async def test_hr_recruitment():
    agent = HRAgent()
    result = await agent.handle_task(
        _make_task("recruitment", {"position": "Engineer", "candidates": ["Alice"]})
    )
    assert result.status == "success"
...
async def test_hr_payroll():
    agent = HRAgent()
    result = await agent.handle_task(
        _make_task("payroll", {"period": "2026-01", "employee_count": 10})
    )
    assert result.status == "success"
...
async def test_finance_invoice():
    agent = FinanceAgent()
    result = await agent.handle_task(
        _make_task("invoice", {"client": "ACME", "amount": 5000})
    )
    assert result.status == "success"
...
async def test_finance_expense():
    agent = FinanceAgent()
    result = await agent.handle_task(
        _make_task("expense", {"category": "travel", "amount": 250})
    )
    assert result.status == "success"
...
async def test_finance_report():
    agent = FinanceAgent()
    result = await agent.handle_task(
        _make_task("report", {"report_type": "monthly", "period": "2026-01"})
    )
    assert result.status == "success"
...
async def test_finance_payroll():
    agent = FinanceAgent()
    result = await agent.handle_task(
        _make_task("payroll", {"period": "2026-01", "employee_count": 50,
                               "total_amount": 250000})
    )
    assert result.status == "success"
    assert result.result["action"] == "payroll_processed"
...
async def test_medical_prescription():
    agent = MedicalAgent()
    result = await agent.handle_task(
        _make_task("prescription", {"medication": "Metformin", "dosage": "500mg"})
    )
    assert result.status == "success"
...
async def test_medical_record():
    agent = MedicalAgent()
    result = await agent.handle_task(
        _make_task("medical_record", {"record_type": "lab_result"})
    )
    assert result.status == "success"
...
async def test_medical_clinical_workflow():
    agent = MedicalAgent()
    result = await agent.handle_task(
        _make_task("clinical_workflow", {"workflow_type": "discharge"})
    )
    assert result.status == "success"
...
async def test_supervisor_routes_to_hr_via_explicit(full_supervisor):
    task = _make_task("process_leave", {"employee_id": "E01", "days": 2},
                      context={"agent_type": "hr"})
...
async def test_supervisor_routes_to_finance_via_task_type(full_supervisor):
    task = _make_task("invoice", {"client": "Corp", "amount": 1000},
                      context={"agent_type": "finance"})
    result = await full_supervisor.handle_task(task)
    assert result.status == "success"
    assert result.result["action"] == "invoice_created"
...
async def test_production_agent_rule_based_fallback(monkeypatch):
    ...
    from agents.production_agent import ProductionAgent, TaskRequest as ProdTaskRequest
...
def test_result_from_state_happy_with_correlation_id():
    state: AgentGraphState = {
        ...
        "status": "success", "result": {"foo": "bar"}, "error": None,
        "created_at": "now", "updated_at": "now",
        "retry_count": 0, "metadata": {}, "correlation_id": "c-001",
    }
    r = result_from_state(state)
    assert r.status == "success"
...
def test_result_from_state_with_error_and_correlation_id():
    state: AgentGraphState = {
        ...
        "status": "failed", "result": {}, "error": "boom",
        "created_at": "now", "updated_at": "now",
        "retry_count": 1, "metadata": {}, "correlation_id": "c-002",
    }
    r = result_from_state(state)
    assert r.error == "boom"
...
def full_supervisor(clean_registry):
    ...
    sup.register_sub_agent(fin)
    sup.register_sub_agent(med)
    return sup
...
async def test_supervisor_rbac_registered_user_denied(clean_registry):
    ...
    from access_control.rbac import Role, RBACEnforcer, RBACUser

    hr  = HRAgent()
    sup = SupervisorAgent(strict_rbac=False)
    sup.register_sub_agent(hr)
    # Register "guest-user" with GUEST role
    sup._rbac.register_user(
        RBACUser(user_id="guest-user", username="Guest User", role=Role.GUEST, tenant_id="t1")
    )
